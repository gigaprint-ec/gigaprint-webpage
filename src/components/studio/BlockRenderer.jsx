...
function SingleBlock({ block, onFormSubmit }) {
  ...
  if (block.type === 'button') return <div className={`public-block block-button align-${block.align || 'left'}`}><a className={`button button-${block.variant || 'primary'}`} href={appHref(block.href)}>{block.label}<ArrowUpRight size={16} /></a></div>;
