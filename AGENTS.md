...
- `src/store.jsx`: estado local, carrito, solicitudes, tema claro/oscuro y tema de temporada.
...
- Sesión admin demo: `gigaprint-admin`.
...
1. Conectar `src/store.jsx` a Supabase con un adaptador separado.
2. Reemplazar la contraseña demo por Supabase Auth y políticas de escritura para usuarios admin.
3. Crear Supabase Storage para imágenes, documentos y archivos de cotización.
4. Sembrar productos, servicios, promociones y bloques iniciales en Supabase.
5. Añadir dominio propio/Vercel cuando se decida el despliegue final.
