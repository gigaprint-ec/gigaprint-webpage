...
import { ArrowRight, ArrowUpRight, BarChart3, Bell, Building2, Calculator, Check, ChevronDown, ChevronRight, ChevronUp, ClipboardList, Clock, Copy, Download, Edit3, ExternalLink, FileCheck, FileText, Filter, Grid, Image, LayoutDashboard, List, MessageCircle, Minus, Package, Palette, Pencil, Plus, RefreshCw, Save, Scissors, Search, Settings2, ShieldCheck, ShoppingCart, SlidersHorizontal, Sparkles, Tag, Trash2, Truck, Users, X, Zap } from 'lucide-react';
...
import { buildWhatsAppCatalogCsv, buildWhatsAppCatalogRow, downloadWhatsAppCatalogCsv } from './lib/whatsappCatalog';
...
function WhatsAppCatalogExportModal({ products, initialProduct, onClose }) {
  const [publicBaseUrl, setPublicBaseUrl] = useState(() => localStorage.getItem('gigaprint-catalog-public-url') || `${window.location.origin}${import.meta.env.BASE_URL || '/'}`.replace(/\/$/, ''));
  const [draft, setDraft] = useState(() => ({ title: initialProduct?.name || '', description: initialProduct?.description || '', price: initialProduct?.price || 0, category: initialProduct?.category || '' }));
  const exportProducts = initialProduct ? [initialProduct] : products;
  const row = initialProduct ? buildWhatsAppCatalogRow(initialProduct, publicBaseUrl, draft) : null;
  const csv = buildWhatsAppCatalogCsv(exportProducts, publicBaseUrl, initialProduct ? { [initialProduct.id]: draft } : {});
  const saveBase = (value) => { setPublicBaseUrl(value); localStorage.setItem('gigaprint-catalog-public-url', value); };
  const download = () => downloadWhatsAppCatalogCsv(csv, `gigaprint-whatsapp-${initialProduct ? initialProduct.id : 'catalogo'}.csv`);
  const copyCsv = async () => { await navigator.clipboard?.writeText(csv); };
  return <div className="modal-backdrop"><div className="admin-modal smart-product-modal whatsapp-export-modal"><div className="modal-heading"><div><span className="eyebrow orange">Catálogo Meta / WhatsApp</span><h2>{initialProduct ? 'Exportar producto' : 'Exportar catálogo'}</h2><small>{initialProduct ? 'Revisa la ficha antes de descargarla.' : `Se exportarán ${products.length} productos del filtro actual.`}</small></div><button onClick={onClose}><X /></button></div><div className="whatsapp-export-intro"><Download size={21} /><div><b>Archivo CSV listo para cargar</b><p>Usa una URL pública HTTPS para que Meta pueda leer las imágenes y las fichas.</p></div></div><div className="smart-product-form"><label>URL pública del sitio<input value={publicBaseUrl} onChange={(event) => saveBase(event.target.value)} placeholder="https://gigaprint-ec.github.io/gigaprint-webpage" /></label>{initialProduct && <div className="fields two"><label>Nombre comercial<input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></label><label>Precio desde (USD)<input type="number" min="0" step="0.01" value={draft.price} onChange={(event) => setDraft({ ...draft, price: Number(event.target.value) })} /></label><label>Categoría Meta<input value={draft.category} onChange={(event) => setDraft({ ...draft, category: event.target.value })} /></label><label>Disponibilidad<select value="in stock" readOnly><option>in stock</option></select></label></div>}{initialProduct && <label>Descripción<textarea rows="4" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} /></label>}{row && <div className="whatsapp-export-preview"><img src={row.image_link} alt="" /><div><span className="eyebrow">Vista previa</span><h3>{row.title}</h3><p>{row.description}</p><strong>{row.price}</strong><small>{row.link}</small></div></div>}{!initialProduct && <div className="whatsapp-export-columns"><span><b>{products.length}</b> fichas</span><span><b>CSV</b> formato feed</span><span><b>HTTPS</b> imágenes públicas</span></div>}<div className="modal-actions"><button className="button button-ghost" onClick={onClose}>Cancelar</button>{initialProduct && <button className="button button-ghost" onClick={copyCsv}><Copy size={15} /> Copiar CSV</button>}<button className="button button-primary" onClick={download}><Download size={16} /> Descargar CSV</button></div></div></div></div>;
}

function SmartAdminProducts() { const { data, removeCollectionItem } = useSite(); const [selected, setSelected] = useState(null); const [exportProduct, setExportProduct] = useState(null); const [exportAll, setExportAll] = useState(false); const [search, setSearch] = useState(''); const [category, setCategory] = useState('Todos'); const visible = data.products.filter((product) => { const term = search.toLowerCase(); return (category === 'Todos' || product.category === category) && `${product.name} ${product.category}`.toLowerCase().includes(term); }).slice(0, 180); const fresh = { id: `product-${Date.now()}`, name: 'Nuevo producto', category: 'Gran formato', type: 'unit', calcType: 'unit', price: 0, unit: 'unidad', image: media.lona, description: 'Descripción del producto', featured: false, isPublished: true, specs: [], variantOptions: [], priceScales: [], colors: [], sizes: [] }; return <AdminShell><AdminHeader eyebrow="Catálogo inteligente" title="Productos, variantes y precios" text={`${data.products.length} productos listos. Edita variables y escalas desde una sola pantalla.`} action={<div className="header-actions"><button className="button button-ghost" onClick={() => setExportAll(true)}><Download size={15} /> Exportar WhatsApp</button><Button onClick={() => setSelected(fresh)}>Añadir producto</Button></div>} /><div className="admin-card catalog-toolbar"><div className="fields two"><label>Buscar producto<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ej. lona, camiseta, tarjeta" /></label><label>Categoría<select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label></div><p>Mostrando {visible.length} de {data.products.length}. El catálogo importado conserva precios por m², unidad, lote y volumen.</p></div><div className="admin-card table-card"><div className="data-table-head"><span>Producto</span><span>Categoría / regla</span><span>Desde</span><span>Acciones</span></div>{visible.map((product) => <div className="data-table-row" key={product.id}><div className="table-product"><img src={product.image} alt="" /><span><b>{product.name}</b><small>{product.variantOptions?.length ? `${product.variantOptions.length} variables · ` : ''}{product.priceScales?.length || 0} escalas</small></span></div><span>{product.category}<br /><small>{getProductCalcType(product) === 'm2' ? 'Por medidas' : product.pricingMode === 'tier-total' ? 'Por lote' : 'Por unidad'}</small></span><strong>{money(product.price)} / {product.unit}</strong><div className="row-actions"><button onClick={() => setExportProduct(product)} title="Exportar a WhatsApp"><Download size={15} /></button><button onClick={() => setSelected(product)} title="Editar variables"><Edit3 size={15} /></button><button className="danger" onClick={() => removeCollectionItem('products', product.id)} title="Eliminar"><Trash2 size={15} /></button></div></div>)}{visible.length === 0 && <div className="admin-empty">No hay productos con esos filtros.</div>}</div>{selected && <SmartProductEditor product={selected} onClose={() => setSelected(null)} />}{(exportProduct || exportAll) && <WhatsAppCatalogExportModal products={visible} initialProduct={exportProduct} onClose={() => { setExportProduct(null); setExportAll(false); }} />}</AdminShell>; }
...
function AdminRoutes() {
  ...
  const pages = {
    ...
    '/admin/whatsapp': <WhatsAppCenterPage />,
