...
export function LogoPicker({ value, onChange }) { const logos = [{ id: 'monogram', label: 'Monograma', node: <span className="picker-logo-mark">G</span> }, { id: 'wordmark', label: 'Wordmark', node: <b className="picker-logo-word">Giga<span>print</span></b> }, { id: 'sticker', label: 'Sticker', node: <span className="picker-logo-sticker">IDEAS<br /><b>GRANDES</b></span> }]; return <div className="studio-logo-grid">{logos.map((logo) => <button type="button" key={logo.id} className={value === logo.id ? 'active' : ''} onClick={() => onChange?.(logo.id)}>{logo.node}<small>{logo.label}</small></button>)}</div>; }
